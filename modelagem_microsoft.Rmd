...
pars_microsoft = expand.grid(ar = 0:2, diff = 0, ma = 0:2)
...
```{r}
# fore_05 = ugarchforecast(fit_05, n.ahead = 5)
# fore_05
```

```{r}
# teste = forecast(modelo_microsoft[[8]])
# modelo_microsoft[[8]]
```
Cross-Validation
x = sample(df_tsible$log_retorno_dif,30)

far2 = function(x, h){
forecast(Arima(x, order=c(1, 0, 2)), h = h)
}

tsCV(x, forecastfunction = far2, h = 7, window = 10)
